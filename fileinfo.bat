...
pushd "%~dp0tools\"
FOR /f "DELIMS=" %%i IN ('.\ffprobe.exe -v error -select_streams v:0 -show_entries stream^=avg_frame_rate -of default^=noprint_wrappers^=1:nokey^=1 "%~1"') DO SET "frame_rate=%%i"
