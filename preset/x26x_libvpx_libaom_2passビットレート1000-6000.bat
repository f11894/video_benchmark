...
for /L %%i in (1000,1000,6000) do call %benchmark_bat% "%~1" "%~n1_libaom_c0_%%ikbps.mp4" "--ivf --cpu-used=0 --tile-columns=3 --tile-rows=3 --end-usage=vbr --passes=2 --target-bitrate=%%i --kf-max-dist=250" libaom c0_2pass_bitrate
for /L %%i in (1000,1000,6000) do call %benchmark_bat% "%~1" "%~n1_x264_placebo_tunessim_%%ikbps.mp4" "--preset placebo --tune ssim --bitrate %%i --pass 2" x264 placebo_tunessim_2pass_bitrate
for /L %%i in (1000,1000,6000) do call %benchmark_bat% "%~1" "%~n1_x265_placebo_tunessim_%%ikbps.mp4" "--preset placebo --tune ssim --bitrate %%i --pass 2" x265 placebo_tunessim_2pass_bitrate
