...
rem For comparison
for /L %%i in (32,-3,18) do call %benchmark_bat% -codec x264 -i "%~1" -o "%~n1_x264_medium_tunessim_crf%%i.mp4" -cmd "--preset medium --tune ssim --keyint 250 --crf %%i" -csvsuf medium_tunessim_crf
for /L %%i in (32,-3,18) do call %benchmark_bat% -codec x265 -i "%~1" -o "%~n1_x265_medium_tunessim_crf%%i.mp4" -cmd "--preset medium --tune ssim --keyint 250 --crf %%i" -csvsuf medium_tunessim_crf
shift
goto loop
:end
exit /b
