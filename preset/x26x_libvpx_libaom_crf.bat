...
for /L %%i in (55,-5,25) do call %benchmark_bat% -codec libvpx -i "%~1" -o "%~n1_libvpx_vp9_c0_2pass_q%%i.webm" -cmd "--codec=vp9 --webm --good --cpu-used=0 --tune=psnr --threads=8 --tile-columns=2 --tile-rows=1 --pass=2 --passes=2 --auto-alt-ref=6 --kf-max-dist=250 --end-usage=q --cq-level=%%i " -csvsuf vp9_c0_2pass_q
for /L %%i in (55,-5,25) do call %benchmark_bat% -codec libaom -i "%~1" -o "%~n1_libaom_c1_2pass_q%%i.mp4" -cmd "--ivf --cpu-used=1 --threads=8 --tile-columns=2 --tile-rows=1 --pass=2 --passes=2 --kf-max-dist=250 --end-usage=q --cq-level=%%i" -csvsuf c1_2pass_q
rem 10bit example
...
rem for /L %%i in (55,-5,25) do call %benchmark_bat% -codec libaom -i "%~1" -o "%~n1_libaom_10bit_c0_2pass_q%%i.mp4" -cmd "--ivf --cpu-used=0 --threads=8 --tile-columns=2 --tile-rows=1 --pass=2 --passes=2 --end-usage=q --cq-level=%%i --kf-max-dist=250 --input-bit-depth=10 --bit-depth=10" -csvsuf 10bit_c0_2pass_q -encode-depth 10
shift
goto loop
:end
exit /b
