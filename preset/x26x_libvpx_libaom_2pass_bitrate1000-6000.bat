...
for /L %%i in (1000,1000,6000) do call %benchmark_bat% -codec libaom -i "%~1" -o "%~n1_libaom_c1_%%ikbps.mp4" -cmd "--ivf --cpu-used=1 --threads=8 --tile-columns=2 --tile-rows=1 --pass=2 --passes=2  --kf-max-dist=250 --end-usage=vbr --target-bitrate=%%i" -csvsuf c1_2pass_bitrate
...
for /L %%i in (1000,1000,6000) do call %benchmark_bat% -codec libvpx -i "%~1" -o "%~n1_libvpx_vp9_c0_2pass_crf%%i.webm" -cmd "--codec=vp9 --webm --good --cpu-used=0 --tune=psnr --threads=8 --tile-columns=2 --tile-rows=1 --pass=2 --passes=2 --auto-alt-ref=6 --kf-max-dist=250 --end-usage=vbr --target-bitrate=%%i" -csvsuf vp9_c0_2pass_bitrate
